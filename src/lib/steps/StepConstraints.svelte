<script>
    ...
    async function handleInput(userInput) {
        updateRawUserInput("constraints", userInput); //store the actual raw input
        isLoading = true;
        displayMessage = "";
        try {
...
<div
    class="flex w-full max-w-2xl min-h-[50rem] justify-between flex-col items-center gap-8 rounded-3xl border border-white/70 bg-white/80 p-6 text-center shadow-2xl shadow-slate-200/70 backdrop-blur-xl sm:p-12 dark:border-slate-700/50 dark:bg-slate-800/40 dark:shadow-none"
>
    <div class="space-y-3">
        <h2
        ...
        >
            Please explain your constraints in brief
        </p>
    </div>
